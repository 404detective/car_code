   ...
   // OutInGarage(OUT_GARAGE, ReadOutInGarageMode()); // 测试出库，拨码在上左侧出入库，反之右侧出入库
    ...
    while (1)
    {
        ...
        if (Camera_Flag == 2)
        {
            ...
            ServoDuty = Servo_Center_Mid - (OFFSET1 + OFFSET2 + OFFSET0) * 1 ;

            // 圆环处理，如果面积为负数，数值越大说明越偏左边；
            if((OFFSET2 < -300)||(OFFSET2 > 300))
                ServoDuty = Servo_Center_Mid - OFFSET2 ;

            ServoCtrl(ServoDuty);     // 舵机PWM输出，转向

            // SPEED正负标识方向，负数为正向
            MotorDuty1 = MtTargetDuty + ECPULSE1 * 4 - (OFFSET1 + OFFSET2 + OFFSET0) / 10;        // 电机PWM
